...
Here we also see just how receptive the wordclouds are in reflecting current events, and how much different subreddits differ in terms of the conversations they are having. For example, HTTPS is featured prominently in the InternetIsBeuatiful subreddit, while terms such as Apple and Iphone only appears in the gadget subreddit. The current event reflection can be seen in how in the world news subreddit, the term ISIS, Russia, Turkey, and Terrorists are all prominent keywords now. We can also see this in the two polar subreddits Conservatives and Liberal. The prominent words there are terms such as Gun, Muslim, Obama, and Carson. We can also see the political leaning of the users of reddit when we visit politics. Bernie Sanders is very frequently mentioned.   Key words played an important role in our prediction model, as we used the key words to predict the different subreddit topics, and we have shown here that they are relevant and important metrics for gauging the thoughts of internet. 
